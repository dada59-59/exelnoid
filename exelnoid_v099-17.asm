...
mouse_enabled   .equ    $C7EB       ; stub (mouse removed)
...
sdm_key_cram:
        call    @cram_load
        ; Check flag in SRAM (reliable) rather than return value
        lda     @cram_loaded
        jnz     @sck_ok
        ; Echec chargement
        mov     %170,A
...
gs_ib:
        ...
        ; Reinstall mouse driver if enabled (game_start may have overwritten it)
        clr     A               ; mouse removed
; mouse reinstall removed
...
; ============================================================================
; CHECK_MOUSE  — poll ExelMouse driver each frame
; ============================================================================
; Driver at $C767 (installed at startup). After call:
;   MOUSE_SWI  ($C7FD): 0=none 1=L 2=R 3=both 4=moved
;   MOUSE_DELTX($C7FE): >128=right <128=left (signed 8-bit as 0-255)
; Must zero MOUSE_SWI after reading.
; ============================================================================
; Trampoline: br to SRAM driver (TASM can't do direct call to equate address)
mouse_call_stub:
        br      @
check_mouse:
        push    A
        push    B
        ; Driver uses TEMP1 (R14/R15) internally — save/restore
        mov     TEMP1-1,A
        push    A
        mov     TEMP1,A
        push    A
        call    @mouse_call_stub    ; call stub → br $C767 → driver RETS back here
        pop     A
        mov     A,TEMP1
        pop     A
        mov     A,TEMP1-1
        lda     @MOUSE_SWI
        jz      @cm_done            ; 0 = no event
        ; Clear event flag immediately
        push    A                   ; save SWI value
        clr     A
        sta     @MOUSE_SWI
        pop     A                   ; restore SWI
        ; Button press (SWI=1,2,3): launch ball if sitting on paddle
        cmp     %4,A
        jhs     @cm_move            ; SWI=4: movement only
        ; SWI=1,2,3: button → launch ball (same as SPACE)
        lda     @ball_active
        jnz     @cm_done            ; already moving
        lda     @game_over
        jnz     @cm_done
        lda     @paused
        jnz     @cm_done
        ...
        jmp     @cm_done
cm_move:
        ; SWI=4: movement — read DELTX
        lda     @MOUSE_DELTX
        jz      @cm_done            ; DELTX=0: no horizontal movement
        cmp     %128,A
        jhs     @cm_right           ; DELTX>128 → right
        ; DELTX < 128 → left
        call    @do_move_left
        jmp     @cm_done
cm_right:
